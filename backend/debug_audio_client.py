#!/usr/bin/env python3
"""
Debug version of audio client with more detailed logging
"""
...
class DebugAudioAssistant:
    """Debug version of AudioAssistant with detailed logging"""
    ...
    async def start(self, input_device=None, output_device=None):
        ...
        try:
            ...
            logger.info("✅ Background audio task created")
    ...
    def _start_audio_streams(self, input_device=None, output_device=None):
        """Start audio input and output streams with detailed logging"""
        try:
            logger.info(f"Opening input stream with device: {input_device}")
            ...
            logger.info(f"Opening output stream with device: {output_device}")
            ...
            logger.error(f"Input device: {input_device}, Output device: {output_device}")
            raise
        ...
        while self.running:
            try:
                ...
                try:
                    ...
                    logger.debug(f"Received audio data, buffer size: {len(audio_buffer)}")
                except queue.Empty:
                ...
                if len(audio_buffer) >= self.CHUNK_SIZE * 2 or (current_time - last_send_time) > 0.1:
                    if audio_buffer:
                        ...
                        logger.debug("Sent audio data to OpenAI")
